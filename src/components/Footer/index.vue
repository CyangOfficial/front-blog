<script setup lang="ts">
</script>

<template>
  <footer class="mx-auto px-3 container xl:max-w-4xl md:(px-0)">
    <p class="my-1 border-b-1 border-b-dotted pb-1 dark:(text-gray-50)">
      Created with <NuxtIcon class="mr-2" name="heart" filled />by Changywa
    </p>
    <div class="flex-between dark:(text-gray-50)">
      <p class="my-1">
        Copyright © 2021 Changywa Inc. and its affiliates.
      </p>
      <p class="my-1 hidden md:(block)">
        © 2020 Changywa&nbsp;<a href="#" class="text-black no-underline dark:(text-gray-50)">京ICP备XXXXX号</a>
      </p>
    </div>
    <!-- <hr class=""> -->
  </footer>
</template>
